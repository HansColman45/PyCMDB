...
public class LaptopHelper {
    ...
    private static LaptopDAO deviceDAO;
    ...
    public static Device createBasicLaptop(Session session, Admin admin, boolean active){
        ...
        AssetType assetType = AssetTypeHelper.createSimpleAssetType(session,category, admin, true);
        ...
        Device laptop = new LaptopBuilder()
            ...
            .withAssetTypeId(assetType.getTypeId())
            .withIdentityId(1)
            .build();
        deviceDAO = new LaptopDAO();
        laptop = deviceDAO.save(session, laptop);

        Log log = new LogBuilder()
            .withDevice(laptop)
            .withLogText("The Laptop with type "+assetType+" is created by Automation in table laptop")
            .build();
