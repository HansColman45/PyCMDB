...
namespace CMDB.UI.Specflow.Actors.Desktops
{
    public class DesktopIdentityActor : DesktopUpdator
    {
        ...
        {
            ExpectedLog = GenericLogLineCreator.AssingDevice2IdenityLogLine($"Desktop with AssetTag: {desktop.AssetTag}",
        ...
        {
            ExpectedLog = GenericLogLineCreator.ReleaseDeviceFromIdentityLogLine($"Desktop with AssetTag: {desktop.AssetTag}",
                $"Identity with name: {identity.Name}", admin.Account.UserID, Table);
